...
ws.merge_cells('B2:C2')
put(ws, 'B2', '内账财务管理系统 · 通用版　使用说明', font=F_TITLE, align=CL, border=None)
ws.row_dimensions[2].height = 30
README = [
 ('适用范围', None),
 ('', '一套开箱即用的 Excel 内账（管理账）系统，适用于贸易、零售、服务、加工等各类中小企业。只录四张表，总账与全部报表自动生成。'),
 ('25 张表怎么分', None),
 ('入口', '首页（仪表盘）· 操作说明 · 使用说明（本页）'),
 ('配置', '科目参数（会计科目 + 收支科目）· 基础资料（账户/部门/员工/商品/客户/供应商）· 期初数据'),
 ('录入（只有这四张手工填）', '资金流水 · 销售录入 · 采购录入 · 其他凭证'),
 ('总账报表', '科目余额表 · 科目分析 · 利润表 · 资产负债表 · 现金流量表'),
 ('业务辅助', '账户余额 · 库存收发存 · 应收跟进 · 应付跟进 · 固定资产台账 · 工资表 · 辅助核算 · 客户对账单 · 供应商对账单 · 经营分析'),
 ('月结', '财务勾稽核查（16 项跨表比对）'),
 ('核心设计', None),
 ('每行业务自带分录对', '每一行录入都会自动算出借方科目、贷方科目和金额，一行同时贡献一借一贷。所以借贷是结构性相等，不可能出现单边挂账。'),
 ('没有写死的科目和年份', '科目表随便加，年度在首页改一格，所有报表自动跟随。科目余额表以「期间 + 科目」为键统一汇总。'),
 ('兼容性', '全部使用 Excel 2007 就有的函数，没有 FILTER / XLOOKUP / TEXTJOIN 等动态数组函数。WPS 个人版、Excel 2016 都能正常打开和重算。'),
 ('快速开始（只做一次）', None),
 ('1. 改首页两格', '会计年度、查询月份。'),
 ('2. 配收支科目', '【科目参数】第二区，每条配好「对方科目」——这笔钱的另一半。这是适配不同行业的主要工作：餐饮加「食材采购」、电商加「平台佣金」、诊所加「药品采购」。'),
 ('3. 建档案', '【基础资料】录账户、部门、员工、商品、客户、供应商。'),
 ('4. 录期初', '【期初数据】录启用日的资产、负债、所有者权益余额。⑤会计科目期初必须借贷相等。'),
 ('5. 核对', '【财务勾稽核查】第 2～7 项全部 ✓ 之后再开始日常录入。'),
 ('每月流程', None),
 ('平时', '收付款 → 资金流水；卖出 → 销售录入；买入 → 采购录入。'),
 ('月末', '工资 / 社保 / 折旧 / 摊销 / 税金计提 → 其他凭证。'),
 ('结账前', '首页切到本月 →【财务勾稽核查】全部 ✓ → 打印报表。'),
 ('年末', '【其他凭证】做结转损益，次年在新文件的【期初数据】录新年度期初。'),
 ('关键口径', None),
 ...
 ('', '不够时在数据区中间插入整行再复制公式，不要在最后一行下面直接粘贴（会盖掉合计行）。'),
 ('颜色约定', None),
 ('', '淡黄底＝手工录入　｜　白 / 灰底＝公式自动计算，请勿覆盖　｜　绿色字＝引用其他工作表　｜　红底＝校验未通过，必须处理。'),
 ('', '工作表标签也按功能分色：深蓝＝入口，灰＝配置，橙＝录入，绿＝总账报表，蓝＝业务辅助，红＝月结检查。'),
 ('注意', None),
 ('不要删整行', '基础资料、期初数据是横向分区共用行号，删整行会同时删掉旁边几个区的数据。清空请选中单元格按 Delete。'),
 ('停用不要删', '停用档案请把「状态」改成停用。'),
 ('年内不做结转损益', '损益科目余额本身就是本年利润，资产负债表已经自动接住。结转损益只在 12 月做一次。'),
]
r = 4
for a, b in README:
    if b is None:
    ...
    r += 1
page(ws, titles=None, landscape=False)
wb[SH_README].sheet_properties.tabColor = '1F3864'
print('  ✓ 使用说明（内置）')
ORDER = [SH_HOME, SH_HELP, SH_README, SH_PARAM, SH_BASE, SH_OPEN,
