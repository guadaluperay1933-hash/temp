...
hdr(6, [('A', '维保状态'), ('B', '台量'), ('C', '占比'), ('D', '免保已到期或\n快到期'),
        ...
        ('M', '提醒'), ('N', '台数')], C_BASE)
TOTL = f'SUM({EA("AD")})'
ws['D6'] = '="免保已到期或"&CHAR(10)&提醒天数&"天内到期"'
for i, st in enumerate(STATES):
    r = 7 + i
    sput(ws, f'A{r}', st, font=F_TXT, fill=FILL_AUTO, align=CL_)
    sput(ws, f'B{r}', f'=SUMIF({EA("W")},$A{r},{EA("AD")})', font=F_IN, fill=FILL_AUTO, fmt=NUM)
...
TYPES = [('客梯', f'SUMIF({EA("H")},"客梯",{EA("AD")})'),
         ('客梯(无机房)', f'SUMIF({EA("H")},"客梯(无机房)",{EA("AD")})+SUMIF({EA("H")},"客梯（无机房）",{EA("AD")})'),
         ('货梯', f'SUMIF({EA("H")},"货梯",{EA("AD")})'), ('扶梯', f'SUMIF({EA("H")},"扶梯",{EA("AD")})'),
         ('人行道', f'SUMIF({EA("H")},"人行道",{EA("AD")})'), ('杂物梯', f'SUMIF({EA("H")},"杂物梯",{EA("AD")})')]
...
sput(ws, 'K7', f'=SUMIF({EA("V")},"?*",{EA("AD")})-$K$8', font=F_IN, fill=FILL_AUTO, fmt=NUM)
sput(ws, 'J8', '写着「未排」', font=F_WARN, fill=FILL_WARN, align=CL_)
sput(ws, 'K8', f'=SUMIF({EA("V")},"未排",{EA("AD")})', font=F_WARN, fill=FILL_WARN, fmt=NUM)
...
sput(ws, 'M15', '↑ 多出来的就是在档案里、但还没挂到任何一张维保组合同上的梯子（或合同台量没填）。档案里台量空着的梯子按 1 台算。',
     font=F_NOTE, align=CL_, border=None)
ws.merge_cells('M15:V15')

# ── ② 各维保组 合同与回款 ──
R2 = 17
sec(R2, ' ② 各维保组：合同与回款（金额＝全部年度累计；项目数、台量、免保项目只算当前合同，不含已续签的旧行；按年度看 ④）', C_RPT)
H2 = [('A', '维保组'), ('B', '项目数\n(当前)'), ('C', '合同台量\n(当前)'), ('D', '合同\n未回'), ('E', '已到期'),
      ('F', '即将\n到期'), ('G', '免保项目\n(当前)'), ('H', '合同应收'), ('I', '合同外\n维修费'), ('J', '总应收'),
...
for i, (sn, _L) in enumerate(TEAMS):
    ...
    F = lambda k: TR(sn, k)
    cells = [('B', f'=COUNTIFS({F("key")},"?*",{F("F")},"<>*已续签*")', NUM),
             ('C', f'=SUMIFS({F("h")},{F("F")},"<>*已续签*")', NUM),
             ('D', f'=COUNTIF({F("F")},"*合同未回*")', NUM), ('E', f'=COUNTIF({F("F")},"*已到期*")', NUM),
             ('F', f'=COUNTIF({F("F")},"*即将到期*")', NUM),
             ('G', '=' + '+'.join(f'COUNTIFS({F("zt")},"{st}",{F("F")},"<>*已续签*")'
                                  for st in ('安装免保', '技术免保', '质保期内')), NUM),
...
CHK = [('应收余额合计 ＝ 总应收 − 已收合计', f'=$T${TT}', f'=ROUND($J${TT}-$O${TT},2)', True,
       ...
       ('开票金额 ＝ 开票已到账 ＋ 开票应收额', f'=$K${TT}', f'=ROUND({TC}{R3 + 4}+{TC}{R3 + 5},2)', True,
        '差出来的＝多开的发票：开票比总应收多，或者开了票的钱记进了「收现金额 / 收款不开票」。组表「回款情况」里会提示「多开票 X，核对发票」。'),
...
DOC = [
    ...
    ('p', '一句话：要开发票的钱记「收款金额」（不管是转账还是现金）；不开发票的现金记「收现金额」；不开发票的转账 / 微信 / 收据记「收款不开票」。'
          '表头上也写了：收款金额(开了票/要开票的)、收现金额(不开票的现金)。'),
    ('p', '开了票的钱千万别记进「收现金额」或「收款不开票」——那样发票就对不上了，「回款情况」会提示「多开票 X，核对发票」，《汇总》⑤ 也会报差。'
          '开票金额比总应收还多，也会这样提示。'),
    ...
    ('p', '「回款情况」一格把这些用一句话说清楚，比如「欠 3,600.00（开票未到账 3,600.00）｜收现 500.00」、「✔ 已结清」、「多收 200.00」。'
          '分期付款还没到收款期的钱写「未到期待收」，不标红；到了期没收的才写「欠」并标红，还会注明「其中已逾期 X」。'),
    ('p', '合同没回来、单价还没填，但已经开了票的：「总应收」先按开票金额暂估，回款情况后面会注明「未填单价，总应收按开票金额暂估」，单价补上以后自动改回按合同算。'),
    ('sec', '三、合同没回来、合同到期'),
    ('p', '「到期提醒」列（F 列）重写了：'),
    ...
    ('p', '　　同一个项目下面已经有止期更晚的新合同行（续签了）→ 旧行显示【已续签】，不再报到期，也不算进《汇总》的「已到期」；'),
    ('p', '　　维保状态选了已解约 → 只显示【已解约】，不再报到期；选了暂停维保 / 免保类 → 在到期提醒后面加上这个标记。'),
    ...
    ('p', '在最右边「付款周期」选：一次性 / 年付 / 半年付 / 季度付 / 月付 / 半月付（原表备注里写了“半年请款一次”“季度付款”“七日内付”的，已经帮你预选好了，核对一下）。'
          '一次性＝整份合同一次付清；年付＝多年合同每年付一次。'),
    ('p', '维保状态是安装免保 / 技术免保 / 质保期内 / 暂停维保 / 已解约的，不排期、不算逾期，欠多少直接看「应收余额」。'),
    ('p', '只要填了合同起止日期，就自动算：'),
    ('p', '　　分期进度 —— 比如「季度付 第3/4期」；'),
    ...
    ('p', '要是像陆莹杨凯表里亿斯源那样「一期拆一行」：每一行的合同起止填这一期自己的起止（比如 7 月 1 日—9 月 30 日），付款周期选一次性，'
          '项目名称四行写成一模一样（「第三季度」这种字写进收款备注），这样四行才会被认成同一个项目，往年欠款、项目累计欠款才串得起来。'),
    ('sec', '六、26 年的欠款接到 27 年（连续多年用）'),
    ('p', '合同续签了，就在同一张组表里新加一行（使用单位、项目名称跟原来一模一样，合同起止写新的），旧的那一行别删。这样：'),
    ('p', '　　新行的「往年欠款」会自动显示这个项目以前各期还没收回来的钱；'),
    ('p', '　　「项目累计欠款」是这个项目所有年份加起来一共还欠多少；'),
    ('p', '　　《汇总》④ 把「统计年度」改成 2027，就能看到 27 年合同的应收 / 已收 / 余额，以及 26 年及以前结转过来的欠款。'),
    ('p', '　　旧行显示【已续签】，不再报到期。'),
    ('p', '钱记在哪一行：27 年收回来的 26 年欠款，照样记在 26 年那一行（收款金额累加，日期写最近一次），这样 26 年那行才会结清；'
          '客户提前付了 27 年的钱，记在 27 年那一行。别把旧欠款记到新行上，不然旧行一直显示欠款、新行显示多收。'),
    ('p', '「使用单位」「项目名称」两格是认项目的钥匙，续签行必须和旧行一字不差（多一个空格、少一个括号都接不上）。'
          '旧行当初只记了项目名、使用单位空着的，续签前先把旧行的使用单位补上，两行写成一样。'),
    ('p', '年度按「合同起始日」算：2026-09-07 开始的合同算 2026 年度。没填起止日的行，归在「没填合同期的」那一列；'
          '但「往年欠款」会把同项目没填起止日的旧行也算进来，不会断。'),
    ...
    ('p', 'Z 列是年检 / 限速器校验 / 载重试验提醒，原表里 2026.09、2027.8、2026.10.1、真日期这几种写法都能认（只写到月的按当月最后一天算）。'
          '月份请写两位：1 月写 2027.01，别写 2027.1——数字 2027.1 和 2027.10 在表格里是同一个数，会被当成 10 月。'),
    ('p', '各维保组表最右边也有「维保状态」下拉，按合同标注，《汇总》② 表会统计免保项目数。'),
    ('sec', '八、顺手修掉的几处原表问题'),
    ('p', '① 到期提醒：没填止期的行（包括空行）原来显示【已到期】，现在不会了。'),
    ('p', '② 总应收、合同应收：好些行漏了公式，合计比实际少（截图里 77,400 对 68,980 就是这个原因），已全部补齐。'
          '所以有两个合计会跟原来不一样：胡春平「合同应收」从 77,400 变成 91,800（欣中泰 4 台原来没公式）；'
          '刘坤田超「合同应收」从 0 变成 10,000（那两行填了单价，但原来没有合同应收公式）。数没丢，是原来少算了。'),
    ('p', '③ 开票应收额：胡春平表后面几行的公式减了「收款日期」，是错的，已统一改掉。'),
    ('p', '④ 应纳税额：原来写死「÷1.06」，改成跟着「税率」那一格走。'),
    ('p', '⑤ 冻结窗格统一成「锁住前三列和前两行」，往右翻收款区的时候单位名称还在。'),
    ('p', '另外：Sheet1 里「滨江苑一期1栋2单元」那两行（2025-08-27~2026-08-26、2026-08-27~2027-08-26 各 6000）就是跨年的标准写法，'
          '这张表格式跟组表不一样，没有动它，也没进汇总；要统计的话按上面第六条搬到陈彪姚永华梅思阳组表里（该项目在那张表第 3 行）。'),
]
r = 3
for kind, txt in DOC:
    if kind == 'sec':
