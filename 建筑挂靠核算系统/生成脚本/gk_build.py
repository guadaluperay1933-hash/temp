...
for i in range(P1 - P0 + 1):
    ...
    put(ws, f'T{r}', f'=IF($A{r}="","",SUMIFS({FA_},{QF}!$D${F0}:$D${F1},"销项开票",'
                     f'{QF}!$C${F0}:$C${F1},$A{r},{QF}!$AI${F0}:$AI${F1},"是"))',
        font=F_LINK, fill=FILL_AUTO, fmt=MONEY)
...
widths(ws, {'A':7,'B':11,'C':10,'D':13,'E':12,'F':12,'G':11,'H':12,'I':14,'J':9,'K':9,'L':34,'M':16,'N':15,
            ...
            'AA':13,'AB':13,'AC':7,'AD':6,'AE':9,'AF':6,'AG':7,'AH':13,'AI':10})
inband(ws, 'A', 'N', 3); sysband(ws, 'O', 'AI', 3)
...
headers(ws, 4, 15, ['项目全称','层级','成本归类','管理费','应开成本票','上游\n税率','我方\n税率',
                    ...
                    '其他应收\n(返现)','其他应付\n(过账/合伙)','年度','代收\n标记','代收台账\n序号',
                    '末层','对业主\n开票','来源表'], fill=FILL_AUTO, font=F_HDR2)
put(ws, 'AI4', '计入汇总', font=F_HDR2, fill=FILL_HDR2)
...
for r in range(F0, F1 + 1):
    ...
    put(ws, f'AF{r}', f'=IF($D{r}<>"销项开票",0,'
        ...
        font=F_LINK, fill=FILL_AUTO)
    # 对业主开票＝这一层的票直接开给业主，是这个工程的收入口径（多条挂靠链各自的顶层都算）
    put(ws, f'AG{r}', f'=IF($D{r}<>"销项开票",0,'
                      f'IF({lk(f"$F{r}", U_NAME, f"{QU}!$C${U0}:$C${U1}")}="业主",1,0))',
        font=F_LINK, fill=FILL_AUTO)
    put(ws, f'AH{r}', None, font=F_LINK, fill=FILL_AUTO)
    put(ws, f'N{r}',
        f'=IF($B{r}="","",'
...
for i, e in enumerate(EV):
    ...
    ws[f'L{r}'] = e['memo'][:120]
    ws[f'M{r}'] = '历史导入'
    ws[f'AH{r}'] = e['src'].split('!')[0].strip()
    ws[f'AI{r}'] = e.get('count_in', '是')
    if e.get('count_in') == '否':
...
for rr in range(F0, F1 + 1):
    if ws[f'AI{rr}'].value is None:
        put(ws, f'AI{rr}', '是', font=F_IN, fill=FILL_IN)
    else:
        put(ws, f'AI{rr}', None, font=F_IN, fill=FILL_IN)
dv_list(ws, f'AI{F0}:AI{F1}', '"是,否"')
...
headers(ws, HR, 1, ['项目编号','项目全称','项目类型','一级单位','合同金额','审计(结算)\n金额',
                    '业主端\n开票额','减:各层\n管理费','减:应提\n税费','加:返现',
                    '票面毛利','毛利率','减:项目\n实际支出','项目净利',
                    '减:应转他方\n(过账/合伙)','归属我方\n利润','状态','有无业务'])
...
for i in range(P1 - P0 + 1):
    ...
    put(ws, f'A{r}', f'=IF({QP}!$A{pr}="","",{QP}!$A{pr})', font=F_LINK)
    put(ws, f'B{r}', f'=IF($A{r}="","",{QP}!$B{pr})', font=F_LINK, align=CL)
    put(ws, f'C{r}', f'=IF($A{r}="","",{QP}!$C{pr})', font=F_TXT, fill=FILL_HDR2)
    put(ws, f'D{r}', f'=IF($A{r}="","",{QP}!$G{pr})', font=F_LINK)
    put(ws, f'E{r}', f'=IF($A{r}="","",N({QP}!$Q{pr}))', font=F_LINK, fmt=MONEY)
    put(ws, f'F{r}', f'=IF($A{r}="","",N({QP}!$R{pr}))', font=F_LINK, fmt=MONEY)
